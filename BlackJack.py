...
class FrozenLakeDQL():
    ...
    def train(self, episodes, render, is_slippery, relu = False, small = False, hidden_layer_size = 16):
        ...
        for i in tqdm.tqdm(range(episodes)):
            ...
            if len(memory)>self.mini_batch_size  and np.max(rewards_per_episode)>0: # TODO  and np.max(rewards_per_episode) > 0
        ...
        print("Reached the goal this many times proportion: ", (rewards_per_episode > 0).sum()/rewards_per_episode.shape)
    ...
    def test(self, episodes, is_slippery=False, render = True, relu = False, small = False, hidden_layer_size = 16):
        ...
        env = gym.make('Blackjack-v1', natural=False, sab=False, render_mode=rendered)
...
if __name__ == '__main__':
    ...
    test_run_number = 100 # How often we let it show what it learned 
    ...
    epoch_number = 10_000 # default: 1_000 (arbitrary)
    ...
    for i in np.arange(number_of_experiments)+1:
        ...
        print("Average success so far: ", sum_of_successes/i) 
        print("")

    #print("Number of successes: ", sum_of_successes)
    print("Average success: ", sum_of_successes/number_of_experiments)
