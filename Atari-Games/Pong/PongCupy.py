...
class PongDQL():
    ...
    def train(self, episodes, render = None, two_layers = False, custom_model = True, convolutional = True, adam = True, hidden_layer_size = 16):
        ...
        plt.subplot(221) 
        plt.plot(cp.asnumpy(rewards_per_episode))
        ...
        for x in range(episodes):
           sum_rewards[x] = cp.sum(cp.asnumpy(rewards_per_episode[max(0, x-100):(x+1)])/((x+1)-max(0, x-100)))
        plt.plot(cp.asnumpy(sum_rewards))
        plt.title("Average reward")
        
        # Plot the loss
        plt.subplot(223)
        plt.plot(cp.array(loss_list).get())
        ...
        if(adam == False):
            plt.subplot(224)
            plt.plot(cp.asnumpy(learning_rate_history))
